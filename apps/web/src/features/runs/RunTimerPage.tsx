...
//
// The UI below is generic: it just draws one arc segment per entry in
// RUN_STRUCTURE, whatever the mix of types, order, or lengths. That matters
// because RUN_STRUCTURE is dummy data standing in for what will eventually
// be an AI-generated plan - a real one might have a longer or shorter run,
// more or fewer rest breaks, or a warmup/cooldown of a different length, and
...
// This run's structure, written out one row at a time so it's easy for a
// human to read straight off the list: a 10-second warmup, six minutes of
// running split into 1-minute segments with a minute of rest after every
// two minutes of running, then a 10-second cooldown.
const RUN_STRUCTURE: RunPhase[] = [
  { type: 'warmup', durationSeconds: 10 },
  { type: 'run', durationSeconds: 1 * 60 },
  { type: 'run', durationSeconds: 1 * 60 },
  { type: 'rest', durationSeconds: 1 * 60 },
  { type: 'run', durationSeconds: 1 * 60 },
  { type: 'run', durationSeconds: 1 * 60 },
  { type: 'rest', durationSeconds: 1 * 60 },
  { type: 'run', durationSeconds: 1 * 60 },
...
// The rest of this file just calls it PHASES, since that's the generic
// "list of segments for this run" the UI actually renders - RUN_STRUCTURE is
// specifically today's hardcoded dummy version of that list.
const PHASES = RUN_STRUCTURE
const TOTAL_SECONDS = PHASES.reduce((sum, phase) => sum + phase.durationSeconds, 0)
...
function RunTimerPage({ runId, onBack, backLabel = 'Back to runs', onComplete }: RunTimerPageProps) {
  ...
  const segments = buildSegments(PHASES, TOTAL_SECONDS)
  const overlayOffset = OVERLAY_LENGTH - OVERLAY_LENGTH * (elapsedSeconds / TOTAL_SECONDS)
